...
function getPolynom(...args) {
  ...
  return (val) => args.reduce((acc, value, index) => acc + value * val ** (args.length - 1 - index), 0);
}
...
function retry(func, attempts) {
  let i = 0;
  return () => {
    while (i <= attempts) { try { return func(); } catch (e) { i += 1; } }
    return i;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  return (...newArgs) => fn.call(null, ...args1, ...newArgs);
}
