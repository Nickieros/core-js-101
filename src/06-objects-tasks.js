...
const cssSelectorBuilder = {
  ...
  combine(selector1, combinator, selector2) {
    return this.copy(`${selector1.stringify()} ${combinator} ${selector2.stringify()}`);
  },
  ...
  copy(value, entityName) {
    const copy = Object.create(this);
    if (['element', 'id', 'pseudoElement'].includes(entityName)) copy[entityName] = this.throwTwiceEx;
    ['element', 'id', 'class', 'attr', 'pseudoClass', 'pseudoElement']
      // eslint-disable-next-line no-return-assign,max-len
      .some((methodName) => (methodName === entityName ? true : (copy[methodName] = this.throwOrderEx, false)));
    copy.selector = value;
    return copy;
  },
};
